...
function datify(date){
    var list = date.split("-");
    return new Date(list[0], list[1], list[2])
}
...
function myFunc(){
    ...
    for (var i = 0; i < activities.length; i++) {
        if (activities[i].checked) {
            ...
            // var date = datify(textlist[1].trim());
            var link = textlist[2].trim();
            var dic = {
                'Date': datify(date),
                'Title': title,
                'Link': link,
            ...
            // });
            var lst = date.split("-");
            var year = parseInt(lst[0]);
            var month = parseInt(lst[1])-1;
            var date = parseInt(lst[2]);
            console.log(year, month, date);
            eventlst.push({'Date': new Date(year, month, date), 'Title': title, 'Link': link});
     ...
     var element = document.getElementById('caleandar');
     element.innerHTML = "";
     caleandar(element, eventlst, settings);
}
